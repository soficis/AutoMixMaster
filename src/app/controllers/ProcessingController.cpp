...
void ProcessingController::runAutoMaster(const domain::Session& session,
  ...
  struct AutoMasterJob final : juce::ThreadPoolJob {
    ...
    JobStatus runJob() override {
      ...
      util::dispatchCallback([capturedCb, result = std::move(result)]() mutable {
  ...
  struct BatchJob final : juce::ThreadPoolJob {
    ...
    JobStatus runJob() override {
      ...
      const auto batchResult = runner.process(
          job,
          [this, capturedCallbacks, &progressMutex, &lastProgressEmit, &lastItemIndex, &lastProgress, &lastStage](
      ...
      util::dispatchCallback([finalCallbacks, result = std::move(result)]() mutable {
